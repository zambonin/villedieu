...
class Graph:
    ...
    def kruskal(self) -> Set[Tuple[int, int, float]]:
        """
        Kruskal's algorithm from CLRS (Introduction to Algorithms, Section
        23.2), using the Timsort algorithm.
        ...
        for edge in edges_sorted:
            if vertex_sets[edge[0]] != vertex_sets[edge[1]]:
                ...
                tmp_vertex_set = vertex_sets[edge[0]].union(
                    vertex_sets[edge[1]]
                )
    ...
    def mst_triple_degree(self) -> Set[Tuple[int, int, float]]:
        """
        Wrapper function that applies the minimal spanning tree algorithm on a
        graph G and sweeps the edge list to locate the vertex with degree 3 or
        more. The sweep is done with each vertex from the edges (undirected
        graph).
...
def random_tests(
    order: int, connections: int, limit: int = 128, tests: int = 1000
):
    """
    ...
    :param limit:       Integer representing the maximum value for all edges
    ...
    for _ in range(tests):
        ...
        Graph(vertexes, edges).mst_triple_degree()
...
def known_tests():
    ...
    # graph which resulting MST has no vertex with degree greater than 3
    graph = Graph(
        {0, 1, 2, 3, 4, 5, 6},
