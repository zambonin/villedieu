...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Recall that a problem $p$ is \textbf{NP}-hard if there exists a polynomial-time, many-one reduction from a \textbf{NP}-complete problem to $p$. Usually, it is said that \textbf{NP}-hard problems are ``at least as hard as the hardest problems in \textbf{NP}'', \emph{i.e.} those that are \textbf{NP}-complete. If a polynomial reduction from a problem $q$ to $p$ exists, 
    \item
    \item
  \end{enumerate}
\end{enumerate}
