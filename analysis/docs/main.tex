...
Let $t_{1}, \dots, t_{7}$ be the number of seconds in each period of the header row and $s = 10^{10}$. The table below solves the equation $t_{i} \leq \frac{f(n)}{s}$ for $n$, that is, it gives $n = f^{-1}(t_{i} s)$. Finding $f^{-1}$ is not trivial for some of the rows. In the case of $n \log_{2} n = c$ for some constant $c$, observe that
\begin{align*}
    n \log_{2} n &= c \\
    n \ln n &= c \ln 2 \\
    e^{\ln n} \ln n &= c \ln 2 \\
