...
\begin{enumerate}
    ...
    \item Let $T(n) = 2T(\frac{n}{2}) + \Theta(n^{2})$. By the recursion-tree method, $T(n) = n^{2} \sum_{i = 0}^{\log_{2} n} 2^{-i} + n$. Using the sum of geometric series, $T(n) = n^{2} \frac{2^{-(\log_{2} n + 1)} - 1}{\frac{1}{2} - 1} + n$, which gives $n^{2}(2 - \frac{1}{n}) + n$, and thus $T(n) \in \Theta(n^{2})$.
        \begin{figure}[htbp]
          ...
          \end{tikzpicture}
        \end{figure}
    \item Let $T(n) = 2T(\frac{n}{3}) + \Theta(n)$. By the recursion-tree method, $T(n) = n\sum_{i = 0}^{\log_{3} n} (\frac{2}{3})^{i} + 2^{\log_{3} n}$. Using the sum of geometric series, $T(n) = n \frac{\frac{2}{3}^{\log_{3} n + 1} - 1}{\frac{2}{3} - 1} + 2^{\log_{3} n}$
        \begin{figure}[htbp]
          \centering
          \begin{tikzpicture}
            \begin{scope}[xshift=-4.75cm]
