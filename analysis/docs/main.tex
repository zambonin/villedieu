...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item $2^{{(\log_{2} n)}^{2}} \in \Omega((\log n)^{\log n})$, for $n_{0} > 1$ and $c = 1$.
            \item $n^{2} \in \Theta(n + n^{2})$, for $n_{0} > 1, c_{1} = \frac{1}{2}$ and $c_{2} = 1$.
            \item $n + (\log n)^{2} \in \Theta(10n + \log_{2} n)$, for $n_{0} > 1, c_{1} = 1$ and $c_{2} = \frac{1}{11}$.
    ...
    \begin{enumerate}
        \item Let $T(n) = 2T(\frac{n}{2}) + \Theta(n)$. By the recursion-tree method, $T(n) = n \sum_{i = 0}^{\log_{2} n} n$. Thus, $T(n) \in \Theta(n \log_2 n)$.
        \begin{figure}[htbp]
          \centering
          \begin{tikzpicture}
    ...
    \begin{enumerate}
        \item 
        \item Due to its recursive nature, it is useful to observe the tree generated by some instance of $f(n)$, where $n = 2^{k}, k \in \mathbb{N}$. It is a perfect tree with height of $\log_{2} n$, since the recursive line halves the number of children, stopping when $n = 2$. For instance, if $n = 8$, the following tree is created.
        \begin{figure}[htbp]
          \centering
          \begin{tikzpicture}
        ...
        \begin{align*}
            ...
            &= \sum_{i = 1}^{\log_{2} n} \frac{n^{i}}{2^{\sum_{j = 1}^{i} j}} \\
            &= \sum_{i = 1}^{\log_{2} n} \frac{n^{i}}{2^{\frac{1}{2}i (i - 1)}} \\
        \end{align*}
    \end{enumerate}
\end{enumerate}
