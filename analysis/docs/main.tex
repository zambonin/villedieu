...
\begin{enumerate}
  ...
  \item Let $T(n) = 3T(\lfloor\frac{n}{2}\rfloor) + \Theta(n)$. By the recursion-tree method, $T(n) = n\sum_{i = 0}^{\lg n - 1} (\tfrac{3}{2})^{i} + n^{\lg 3}$. As per Exercise~\ref{ex:4d}, $T(n) \in \Theta(n^{\lg 3})$.
    \begin{figure}[htbp]
      ...
      \end{tikzpicture}
    \end{figure}
  \item Let $T(n) = 4T(\lfloor\frac{n}{2}\rfloor) + \Theta(n)$. By the recursion-tree method, $T(n) = n\sum_{i = 0}^{\lg n - 1} 2^{i} + n^{2} $. Using the sum of geometric series,
  \begin{align*}
    T(n) & = n\tfrac{2^{\lg n - 1 + 1} - 1}{2 - 1} + n^{2} \\
         & = n(2^{\lg n} - 1) + n^{2} \\
         & = n^{2} + nn^{\lg 2} + n
         & = 2n^{2} + n
    T(n) & \in \Theta(n^{2}).
  \end{align*}
  $T(n) = n\frac{2^{\lg n - 1 + 1} - 1}{2 - 1} + n^{2}$, which gives $n(n - 1) + n^{2}$, and thus $T(n) \in \Theta(n^{2})$.
    \begin{figure}[htbp]
      \centering
      \begin{tikzpicture}
      ...
      \end{tikzpicture}
    \end{figure}
  \item Let $T(n) = 2T(\frac{n}{2}) + \Theta(n^{2})$. By the recursion-tree method, $T(n) = n^{2} \sum_{i = 0}^{\lg n - 1} 2^{-i} + n$. Using the sum of geometric series, $T(n) = n^{2} \frac{2^{-(\lg n - 1 + 1)} - 1}{\frac{1}{2} - 1} + n$, which gives $2n^{2} - n$, and thus $T(n) \in \Theta(n^{2})$.
    \begin{figure}[htbp]
      \centering
      \begin{tikzpicture}
      \begin{scope}[xshift=-4.75cm]
  ...
  \begin{enumerate}
    ...
    \begin{align*}
      ...
      T(n) &\in \Theta(n^{2}).
    \end{align*}
    \item Due to its recursive nature, it is useful to observe the tree generated by some instance of $f(n)$, where $n = 2^{k}, k \in \mathbb{N}$. It is a perfect tree with $\lg n$ levels, since the recursive line halves the number of children, stopping when $n = 2$. For instance, if $n = 8$, the following tree is created.
    \begin{figure}[htbp]
      \centering
      \begin{tikzpicture}
