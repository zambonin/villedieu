...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            ...
            T(n) &\in \Theta(2^{n}).
        \end{align*}
        To prove the final formula by weak induction, let $T(n) = 2^{n}(n - 1)$. $T(1)$ holds, since $2^{1}(1 - 1) = 0$. The inductive step is proved below, using the hypothesis that $T(n - 1)$ is true.
        \begin{align*}
            T(n - 1) + 2^{n} &\stackrel{?}{=} T(n) \\
            2^{n - 1}((n - 1) - 1) + 2^{n} &\stackrel{?}{=} \\
            2^{n - 1}n &\stackrel{?}{=}
        \end{align*}
        \item Let \[
            T(n) = \begin{cases}
        ...
        \begin{align*}
            ...
            &= c^{i} T(n - i).
        \end{align*}
        Consider $T(n - i) \Leftrightarrow T(1)$. Thus, $n - i = k$ and $i = n - k$, which gives
        \begin{align*}
            T(n) &= c^{n - k} T(n - (n - k))  \\
            &= c^{n - k} T(k) \\
            T(n) &\in \Theta(c^{\mathcal{O}(n)}).
        \end{align*}
        \item Let \[
            T(n) = \begin{cases}
                1 &\text{if } n = 1, \\
