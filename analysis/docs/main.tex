...
\begin{enumerate}
    ...
    \item
    \begin{enumerate}
        \item Let $T(n) = T(\frac{n}{2}) + \Theta(1)$, with $a = 1, b = 2$ and $f(n) \in \Theta(1)$. Since $n^{\log_{1} 2} = 1$ and $f(n) \in \Theta(n^{\log_{1} 2})$, the second case may be applied. Thus, $T(n) \in \Theta(\log_{2} n)$.
        \item Let $T(n) = T(\frac{9n}{10}) + n$, with $a = 1, b = \frac{10}{9}$ and $f(n) \in \Theta(n)$. We have that $n^{\log_{1} \frac{10}{9}} = 1$, $f(n) \in \Omega(n^{\log_{1} \frac{10}{9} + \epsilon})$ and $\frac{n}{2} \leq cn$ with $\epsilon = c = \frac{3}{4}$. Thus, the third case may be applied and $T(n) \in \Theta(n)$.
    \end{enumerate}
\end{enumerate}
