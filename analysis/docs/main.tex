...
Let $t_{1}, \dots, t_{7}$ be the number of seconds in each period of the header row and $s = 10^{10}$. The table below solves the equation $t_{i} \leq \frac{f(n)}{s}$ for $n$, that is, it gives $n = f^{-1}(t_{i} s)$. Finding $f^{-1}$ is not trivial for some of the rows. In the case of $n \log_{2} n$,


% As per~\cite{Cantrell:200110:misc}, 
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l*{7}{r}}
        \toprule
            ...
            $n \log_{2} n$ & $\euler^{W(t_{1} s \ln_2)}$ & $\euler^{W(t_{2} s \ln_2)}$ & $\euler^{W(t_{3} s \ln_2)}$ & $\euler^{W(t_{4} s \ln_2)}$ & $\euler^{W(t_{5} s \ln_2)}$ & $\euler^{W(t_{6} s \ln_2)}$ & $\euler^{W(t_{7} s \ln_2)}$ \\
