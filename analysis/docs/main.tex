...
\begin{enumerate}
    ...
    \item
    \begin{enumerate}
        \item Consider that $I$ is a set of $2$-uples $I_{j} : (s, e), j \in \{1, \dots, n\}$ such that $s, e \in \mathbb{Z}^{+}, s \leq e$. To order such a set, the condition $I_{j - 1}.e < I_{j}.s$ has to be met for all $j \in \{2, \dots, n\}$. Additionally,
