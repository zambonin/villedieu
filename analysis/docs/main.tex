...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item 
        \item Due to its recursive nature, it is useful to observe the tree generated by some instance of $f(n)$, where $n = 2^{k}, k \in \mathbb{N}$. It is a perfect binary tree with height of $\log_{2} n$, since the recursive line halves the input size, and consequently the number of children, stopping when $n = 2$. For instance, if $n = 8$, the following tree is generated.
        \begin{figure}[htbp]
          \centering
          \begin{tikzpicture}
        ...
        %   \end{scope}
          \end{tikzpicture}
        \end{figure}
        The costs are given numerically below.
        \begin{align*}
            T(1) &= 0 \\
            T(2) &= 2 + (2 * 0) &\Rightarrow 2 + (2 * T(1)) \\
            T(4) &= 4 + (4 * (2 + (2 * 0))) &\Rightarrow 4 + (4 * T(2)) \\
            T(8) &= 8 + (8 * (4 + (4 * (2 + (2 * 0))))) &\Rightarrow 8 + (8 * T(4)) \\
            &\vdots \\
            T(n) &= n + (n * T(\tfrac{n}{2})) &\Rightarrow n * (1 + T(\tfrac{n}{2}))
        \end{align*}
    \end{enumerate}
\end{enumerate}
