...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            ...
            &= \sum_{i = 1}^{n} 2^{i}.
        \end{align*}
        The sum of this geometric series is exactly $\frac{2^{n + 1} - 1}{2 - 1} - 2^{0} = 2^{n + 1} - 2$, since $i$ does not start at $0$. To prove the final formula by weak induction, let $T(n) = 2^{n + 1} - 2$. $T(1)$ holds, since $2^{1 + 1} - 2 = 0$. The inductive step is proved below, using the hypothesis that $T(n - 1)$ is true.
        \begin{align*}
