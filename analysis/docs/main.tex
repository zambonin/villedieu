...
\begin{enumerate}
    ...
    \item Let $T(n) = 3T(\lfloor\frac{n}{2}\rfloor) + \Theta(n)$. By the recursion-tree method, $T(n) = n\sum_{i = 0}^{\log_{2} n} (\tfrac{3}{2})^{i} + n^{\log_{2} 3}$. As per Exercise~\ref{ex:4d}, $T(n) \in \Theta(\text{poly}(n))$.
        \begin{figure}[htbp]
          ...
          \end{tikzpicture}
        \end{figure}
    \item Let $T(n) = 4T(\lfloor\frac{n}{2}\rfloor) + \Theta(n)$. By the recursion-tree method, $T(n) = n\sum_{i = 0}^{\log_{2} n} 2^{i} + n^{2} $. Using the sum of geometric series, $T(n) = n\frac{2^{\log_{2} n + 1} - 1}{2 - 1} + n^{2}$, and thus $T(n) \in \Theta(n^{2})$.
        \begin{figure}[htbp]
          \centering
          \begin{tikzpicture}
          ...
          \end{tikzpicture}
        \end{figure}
    \item Let $T(n) = 2T(\frac{n}{2}) + \Theta(n^{2})$. By the recursion-tree method, $T(n) = n^{2} \sum_{i = 0}^{\log_{2} n} 2^{-i} + n$. Using the sum of geometric series, $T(n) = n^{2} \frac{2^{-(\log_{2} n + 1)} - 1}{\frac{1}{2} - 1} + n$.
        \begin{figure}[htbp]
          \centering
          \begin{tikzpicture}
            \begin{scope}[xshift=-4.75cm]
